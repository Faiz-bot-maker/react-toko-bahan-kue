...
const adminMenus = [
  ...
  },
  {
    ...
    icon: <HiOutlineDocumentReport className="text-sm" />,
    items: [
      { name: 'Laporan Barang Terlaris', path: '/admin/laporan-produk-terlaris' },
      ...
      { name: 'Laporan Transaksi Keluar', path: '/admin/adm-transaksi-keluar' },
      { name: 'Laporan Transaksi Masuk', path: '/admin/transaksi-masuk' },
      { name: 'Laporan Laba Rugi', path: '/admin/report-loss' },
    ],
  },
  {
    section: 'Keuangan',
  ...
  return (
    <aside className="w-60 h-screen bg-[#11493E] shadow-xl flex flex-col overflow-hidden">
      ...
      >
        <Link
          to="/dashboard"
          className={ `flex items-center px-6 py-2 rounded-md gap-2 font-medium text-xs transition-all duration-200 hover:bg-[#1B5E4B] hover:text-white ${location.pathname === '/dashboard'
              ...
              <ul className="flex flex-col gap-1 mt-1 pl-2 border-l border-[#1B5E4B] ml-4">
                { menuSection.items.map( ( item ) => (
                  <li key={ item.path }>
                    <Link
                      to={ item.path }
                      className={ `flex items-center px-8 py-1.5 rounded-md text-[11px] transition-all duration-200 hover:bg-[#1B5E4B] hover:text-white ${location.pathname === item.path
