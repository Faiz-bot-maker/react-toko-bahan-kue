...
const OwnerKeuangan = () => {
  ...
  const fetchSummary = async () => {
    try {
      ...
      if (summaryStart && summaryEnd) {
        params.start_date = summaryStart.toISOString().split("T")[0];
        params.end_date = summaryEnd.toISOString().split("T")[0];
      }
  ...
  // Fetch Balance Sheet awal (semua tanggal jika null)
  useEffect(() => {
    fetchBalanceSheet();
  }, [selectedDate]);

  // Fetch Cash Flow awal (semua periode jika null)
  useEffect(() => {
    fetchCashFlow();
  }, [cashFlowRange]);
        ...
        {summary && (
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
            <div className="bg-white border rounded-xl shadow p-6">
              ...
              <p className="text-2xl font-bold text-green-600 mt-2">Rp {summary.total_sales.toLocaleString("id-ID")}</p>
            </div>
            <div className="bg-white border rounded-xl shadow p-6">
              <h3 className="text-sm font-semibold text-gray-600">Total Biaya</h3>
              <p className="text-2xl font-bold text-red-600 mt-2">Rp {summary.total_expenses.toLocaleString("id-ID")}</p>
            </div>
            <div className="bg-white border rounded-xl shadow p-6">
              <h3 className="text-sm font-semibold text-gray-600">Laba Bersih</h3>
              <p className="text-2xl font-bold text-blue-600 mt-2">Rp {summary.net_profit.toLocaleString("id-ID")}</p>
            </div>
          </div>
        )}
          ...
          <div className="mb-4 flex items-center gap-2">
            ...
            <button
              onClick={() => {
                ...
                setBalanceSheet(null); // Reset tabel
              }}
