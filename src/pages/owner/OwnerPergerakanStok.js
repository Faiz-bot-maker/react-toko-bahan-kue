...
const OwnerPergerakanStok = () => {
    ...
    // auto fetch everytime filters updated
    useEffect(() => {
        if ((startDate && !endDate) || (!startDate && endDate)) return;
        fetchMovements(currentPage, searchTerm);
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [startDate, endDate, branchFilter, typeFilter, currentPage, searchTerm]);
    ...
    const fetchSummary = async () => {
        try {
            ...
            setSummaryTotal(res.data?.total_all_branches || null);
        } catch (err) {
        ...
        <Layout>
            <div className="w-full max-w-7xl mx-auto">
                ...
                <div className="flex items-center justify-between mb-8">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-gray-600">Catatan perubahan stok barang per cabang</p>
                        </div>
                    </div>
                </div>
                ...
                <div className="bg-white rounded-lg shadow mb-6 p-4">
                    <h2 className="text-lg font-bold text-gray-800 mb-3">Ringkasan Pergerakan Stok</h2>
                        ...
                        <table className="min-w-full text-sm">
                            <thead>
                                <tr className="bg-gray-800 border-b">
                                    ...
                                    <th className="px-4 py-2 text-left font-semibold text-green-100">Total IN</th>
                                    <th className="px-4 py-2 text-left font-semibold text-red-100">Total OUT</th>
                                </tr>
                            </thead>
                            <tbody>
                                {summary.length === 0 ? (
                                    ...
                                    summary.map((row, idx) => (
                                        <tr key={idx} className="border-b hover:bg-gray-50">
                                            ...
                                            <td className="px-4 py-2 font-semibold text-green-600">{row.total_in}</td>
                                            <td className="px-4 py-2 font-semibold text-red-600">{row.total_out}</td>
                                        </tr>
                                    ))
                                )}
                            </tbody>
                    ...
                    {summaryTotal && (
                        <div className="mt-4 p-3 bg-gray-50 rounded border text-sm">
                            <p className="font-medium text-gray-700">Total Semua Cabang:</p>
                            <p className="text-green-700">Total IN: {summaryTotal.total_in}</p>
                            <p className="text-red-700">Total OUT: {summaryTotal.total_out}</p>
                        </div>
                    )}
                </div>
                        ...
                        <table className="min-w-full">
                            <thead className="bg-gradient-to-r from-gray-700 to-gray-800 text-white">
                                <tr>
                                    ...
                                    <th className="px-6 py-4 text-left text-xs font-semibold uppercase tracking-wider">Produk</th>
                                    <th className="px-6 py-4 text-left text-xs font-semibold uppercase tracking-wider">Ukuran</th>
                                    <th className="px-6 py-4 text-left text-xs font-semibold uppercase tracking-wider">Referensi</th>
                                    <th className="px-6 py-4 text-left text-xs font-semibold uppercase tracking-wider">Perubahan</th>
                                ...
                                {loading ? (
                                    ...
                                    movements.map((mv) => (
                                        <tr key={mv.id} className="hover:bg-gray-50">
                                            ...
                                            <td className="px-6 py-4 text-gray-600">{formatDate(mv.created_at)}</td>
                                        </tr>
                                    ))
                                )}
                            </tbody>
