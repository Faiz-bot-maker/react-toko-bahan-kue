...
//   // ✅ Sudah diperbaiki untuk langsung menambah product + size sekaligus
...
//       // 2️⃣ Siapkan sizes untuk dikirim
...
//       // 3️⃣ Kirim sizes jika ada
//       if (sizesToCreate.length > 0) {
//         await axios.post(`${API_URL}/${createdProduct.sku}/sizes`, sizesToCreate, { headers: getHeaders() });
//       }

//       // 4️⃣ Reset form & redirect
...
const Products = () => {
  ...
  const handleAddProduct = async () => {
    try {
      // Siapkan data produk dan ukuran
      const productData = {
        name: newProduct.name,
        category_id: newProduct.category_id,
        sku: `TKAZ-${newProduct.sku}`,
        sizes: newProduct.sizes
          .filter(s => s.name.trim() !== '') // Hanya ukuran dengan nama yang valid
          .map(s => ({
            name: s.name,
            buy_price: parseInt(s.buy_price) || 0,
            sell_price: parseInt(s.sell_price) || 0
      ...
      // Arahkan ke halaman detail jika diperlukan
      navigate(`/pages/sizeproduct?sku=${createdProduct.sku}`);
  ...
  // Render fungsi tetap sama seperti sebelumnya
  return (
    <div className="flex h-screen bg-gradient-to-tr from-white via-blue-50 to-jade-50">
      <Sidebar />
      <div className="flex-1 flex flex-col min-w-0">
