...
const AdminModal = ( { branchId } ) => {
    ...
    const [ form, setForm ] = useState( {
        ...
        note: "",
        amount: "",
        branch_id: branchId, // otomatis pakai cabang admin
    } );
    const [ loading, setLoading ] = useState( true );

    useEffect( () => {
        if ( branchId ) fetchModals( branchId );
    }, [ branchId ] );

    const fetchModals = async ( branchId ) => {
        try {
            setLoading( true );
    ...
    const openAdd = () => {
        setForm( { id: null, type: "OUT", note: "", amount: "", branch_id: branchId } );
