...
const AdminLaporanPiutang = () => {
    ...
    const fetchPelanggan = async () => {
        try {
            ...
            // Filter data hanya cabang admin jika ada field branch_id
            const adminBranchId = localStorage.getItem("branch_id"); // contoh
            const filtered = adminBranchId ? data.filter(d => d.branch_id == adminBranchId) : data;
            if (Array.isArray(filtered)) setPelanggan(filtered);
        } catch (err) {
                ...
                <div className="flex items-center justify-between mb-8">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-gray-600">Kelola data piutang cabang Anda</p>
                        </div>
                    </div>
                    <button 
                        onClick={openAdd} 
                                ...
                                {loading ? (
                                    ...
                                    pelanggan.map((p, idx) => (
                                        <tr key={idx} className="hover:bg-gray-50 transition-colors">
                                            ...
                                            <td className="px-6 py-4 text-gray-700 font-semibold">{formatRupiah(p.total)}</td>
                            ...
                            <div className="p-6">
                                ...
                                <form onSubmit={handleSubmit} className="space-y-6">
                                    <div>
                                        ...
                                        <input 
                                            type="text" 
                                            className="w-full border border-gray-300 px-4 py-3 rounded-lg focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-red-500 text-sm" 
                                    ...
                                    <div className="flex justify-end gap-3 pt-4 border-t border-gray-200">
                                        <button type="button" onClick={closeModal} className="px-6 py-2.5 border border-gray-300 text-gray-700 rounded-lg font-medium hover:bg-gray-50 transition-colors">
                                            Batal
                                        </button>
                                        <button type="submit" className="px-6 py-2.5 bg-green-600 hover:bg-green-700 text-white rounded-lg font-medium transition-colors">
                                            Simpan
                                        </button>
                                    </div>
                                </form>
