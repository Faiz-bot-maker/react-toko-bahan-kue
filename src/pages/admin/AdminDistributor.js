...
const AdminDistributor = () => {
    ...
    const [form, setForm] = useState({ id: null, name: "", address: "", branch_id: null });
    const [loading, setLoading] = useState(true);

    const branchId = localStorage.getItem("branchId"); // cabang admin
    ...
    const fetchDistributors = async () => {
        try {
            setLoading(true);
            const res = await axios.get(`${API_URL}?branch_id=${branchId}`, {
                headers: {
    ...
    const openAdd = () => {
        setForm({ id: null, name: "", address: "", branch_id: Number(branchId) });
        setModal({ open: true, mode: "add", idx: null });
    };

    const openEdit = (idx) => {
        const d = distributors[idx];
        setForm({ id: d.id, name: d.name, address: d.address, branch_id: Number(branchId) });
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const payload = { name: form.name, address: form.address, branch_id: Number(branchId) };
            const headers = {
                Authorization: localStorage.getItem("authToken"),
                "ngrok-skip-browser-warning": "true",
                                    ...
                                    <tr>
                                        <td colSpan={3} className="px-6 py-16 text-center">
                                            <div className="flex flex-col items-center">
                                                ...
                                                <p className="text-gray-500 mb-4 text-sm">Mulai dengan menambahkan distributor cabang Anda</p>
                                                <button
                                                    onClick={openAdd}
                        ...
                        <div className="bg-white rounded-lg shadow-2xl w-full max-w-md mx-4 border border-gray-200">
                            <div className="p-6">
                                ...
                                <form onSubmit={handleSubmit} className="space-y-6">
                                    <div>
                                        <label className="block text-sm font-semibold text-gray-700 mb-2">Nama Distributor</label>
                                        <input
                                            type="text"
